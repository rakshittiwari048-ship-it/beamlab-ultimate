 ...
 * ViewerPage.tsx
 *
 * Example integration of StructuralCanvas with ModelingSidebar
 * Demonstrates:
 * - Loading models from factory service
 * - Rendering structure in 3D canvas
 * - Handling member selection and hovering
...
export const ViewerPage: React.FC = () => {
  ...
  const handleModelLoad = useCallback((factoryModel: StructuralModel) => {
    ...
    const nodes: StructuralNode[] = factoryModel.nodes.map((n) => ({
      ...
      support: false, // Could be enhanced with support detection
      label: n.id,
    }));
  ...
  const selectedMember = modelState.members.find((m: StructuralMember) => m.id === selectedMemberId);
  const hoveredMember = modelState.members.find((m: StructuralMember) => m.id === hoveredMemberId);
      ...
      {selectedMember && (
        <div className="w-80 border-l border-zinc-800 bg-zinc-800/50 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            ...
            {/* Length */}
            <div className="bg-zinc-700/30 rounded px-3 py-2">
              <p className="text-xs font-semibold text-zinc-300">Length</p>
